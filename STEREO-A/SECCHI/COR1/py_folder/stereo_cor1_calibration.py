    ...
    def rotate_image_solar_north_up(image, header, missing_value=0.0,
        ...
        if 'CROTA2' in header:
            ...
            if obstime:
                ...
                roll_angle = STEREOCoordinates.get_stereo_roll_angle(obstime, observatory)
        ...
        ny, nx = image.shape
        solar_center_x, solar_center_y = STEREOCoordinateTransform.get_solar_center_pixel(
            header, image.shape, origin=0
        )

        array_center_x = (nx - 1) / 2.0
        array_center_y = (ny - 1) / 2.0

        shift_x = array_center_x - solar_center_x
        shift_y = array_center_y - solar_center_y
        ...
        updated_header['HISTORY'] = (
            f'Applied solar north rotation about WCS solar center: {rotation_angle:.3f} deg'
        )
...
class STEREOCOR1Calibration:
    ...
    def apply_smooth_mask(self, image, header, fill_value=0.0):
        ...
        rsun_pix = None
        if 'RSUN' in header:
            rsun_value = float(header['RSUN'])

            cdelt_candidates = []
            if 'CDELT1' in header:
                cdelt_candidates.append(abs(float(header['CDELT1'])))
            if 'CDELT2' in header:
                cdelt_candidates.append(abs(float(header['CDELT2'])))

            if cdelt_candidates and np.isfinite(rsun_value):
                cdelt_mean = float(np.mean(cdelt_candidates))
                if cdelt_mean > 0:
                    rsun_pix = rsun_value / cdelt_mean

            if rsun_pix is None or (not np.isfinite(rsun_pix)) or rsun_pix <= 0:
                rsun_pix = rsun_value

        if rsun_pix is None or (not np.isfinite(rsun_pix)) or rsun_pix <= 0:
            rsun_pix = min(nx, ny) / 20.0

        min_to_edge = min(center_x, center_y, (nx - 1) - center_x, (ny - 1) - center_y)
        if not np.isfinite(min_to_edge) or min_to_edge <= 0:
            min_to_edge = min(nx, ny) / 2.0

        inner_radius = rsun_pix * 1.1
        outer_radius = 0.95 * min_to_edge

        mask = np.ones_like(image, dtype=bool)
        mask[r < inner_radius] = False
        mask[r > outer_radius] = False

        masked_image = image.copy()
        masked_image[~mask] = fill_value

        self.log_message(
            f"Applied smooth mask (WCS center=({center_x:.2f},{center_y:.2f}), "
            f"rsun_pix={rsun_pix:.2f}, inner={inner_radius:.2f}, "
            f"outer={outer_radius:.2f}, valid_pixels={np.sum(mask)})"
        )
        return masked_image
