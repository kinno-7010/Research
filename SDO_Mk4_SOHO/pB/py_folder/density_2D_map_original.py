...
# ---------------------------------
def invert_pb_to_density_axisymmetric(pb_image,
                                      r_map_rsun,
                                      params_ref,
                                      r_min,
                                      r_max,
...
def invert_pb_to_density_spherical(pb_image,
                                   r_map_rsun,
                                   params_ref,
                                   r_min,
                                   r_max,
...
def invert_pb_to_density_2D(pb_image,
                            r_map_rsun,
                            params_ref,
                            r_min,
                            r_max,
    ...
    if sym in ("axisymmetric", "axis", "axi"):
        return invert_pb_to_density_axisymmetric(pb_image, r_map_rsun, params_ref,
                                                 r_min=r_min, r_max=r_max, **kwargs)
    elif sym in ("spherical", "sph", "sphere"):
        return invert_pb_to_density_spherical(pb_image, r_map_rsun, params_ref,
                                              r_min=r_min, r_max=r_max, **kwargs)
    else:
...
if __name__ == "__main__":
    ...
    r_edges = build_adaptive_radial_edges(
        ...
        inner_px_factor=3.0,   # Mk4 側は ~2px 程度の厚みに近づける
        outer_px_factor=2.0,   # LASCO 側は ~2px 程度の厚みに近づける
        min_dr=0.01,
        max_dr=0.25
    )
    ...
    if SYMMETRY.lower().startswith("sph"):
        ...
        density_map, aux = invert_pb_to_density_2D(
            ...
            symmetry="spherical",
            r_edges=r_edges,              # r-edges を明示
            spatial_fill_mode="nearest",  # ごく最小限の穴埋め
            spatial_fill_iters=1
        )
        title = "K-COR+SOHO/LASCO-C2 Electron Density (Spherical symmetric inversion)\n2022-06-13 03:01:00 UT"

        suffix = "sph"
    else:
        # 軸対称：セクタ毎に pB(r) を反転 → 2D へ展開
        # 「Mk4+LASCO のビン段差塗り」に揃えるため use_bilinear=False を推奨
        density_map, aux = invert_pb_to_density_2D(
            pb_image=final_pb,
            r_map_rsun=r_map_lasco,
            params_ref=params_lasco,
            r_min=r_min, r_max=r_max,
            symmetry="axisymmetric",
            theta_step_deg=1.0,           # extract_pB_profile(±5°)とタイル一致
            r_edges=r_edges,              # 非一様ビンに対応
            use_bilinear=False,           # ★ 段差塗り（nearest bin）
            theta_neighbor_blend=2,       # 角度近傍の最小限ブレンド（0..2程度）
            theta_neighbor_fallback=6,    # 残欠損の θ 近傍フォールバック
            spatial_fill_mode="nearest",  # 2D 最近傍での最小限穴埋め
            spatial_fill_iters=1
        )
        title = "Converted Density (Axisymmetric inversion)\n2022-06-13 03:01:00 UT"
        suffix = "axi"
    ...
    print(f"Saved 2D density map to {png_path}")

    export_density_csv(density_map, r_map_lasco, params_lasco, csv_path, include_all=False)
