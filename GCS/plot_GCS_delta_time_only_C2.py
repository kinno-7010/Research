...
integrated_analysis = importlib.import_module("integrated_analysis")  # noqa: E402
IA_CONFIG = integrated_analysis.config
scan_directory_for_maps = integrated_analysis.scan_directory_for_maps
select_lasco_by_time_threshold = integrated_analysis.select_lasco_by_time_threshold
create_fully_corrected_lasco_map = integrated_analysis.create_fully_corrected_lasco_map
ia_get_params = integrated_analysis.get_params
...
def _create_lasco_only_diff_image(ax, target_time_str: str, delta_time: int = 13):
    """LASCO差分のみを生成してプロットする."""
    from astropy.time import Time
    ...
    earliest_time = min(target_time_obj, base_time_obj)
    latest_time = max(target_time_obj, base_time_obj)
    scan_start = earliest_time - 10 * u.min
    scan_end = latest_time + 10 * u.min

    lasco_dir = IA_CONFIG.data_folder_dict.get('lasco')
    if not lasco_dir:
        raise ValueError("LASCOデータディレクトリが config.data_folder_dict に設定されていません。")

    lasco_dir = Path(lasco_dir)
    lasco_list = scan_directory_for_maps(lasco_dir, scan_start.iso, scan_end.iso, use_cache=True)
    if not lasco_list:
        raise ValueError(f"LASCOデータがスキャン範囲 {scan_start.iso} - {scan_end.iso} に存在しません。")

    lasco_shapes = []
    ref_map = None
    sample_size = min(3, len(lasco_list))
    for _, path in lasco_list[:sample_size]:
        try:
            ...
            if ref_map is None:
                ...
                if temp_map.data.size > ref_map.data.size:
                    ref_map = temp_map
            del temp_map
            gc.collect()
        except Exception as exc:  # pragma: no cover - diagnostic
            print(f"警告: LASCOマップ読み込みエラー {path}: {exc}")

    if not lasco_shapes or ref_map is None:
    ...
    target_entry, _ = select_lasco_by_time_threshold(target_time_obj, lasco_list)
    base_entry, _ = select_lasco_by_time_threshold(base_time_obj, lasco_list)

    print(f"LASCO target時刻: {target_entry.date.iso}")
    print(f"LASCO base時刻: {base_entry.date.iso}")

    target_map_corrected = create_fully_corrected_lasco_map(target_entry)
    base_map_corrected = create_fully_corrected_lasco_map(base_entry)

    lasco_map = _resample_map(target_map_corrected)
    lasco_map_base = _resample_map(base_map_corrected)

    p_lasco = ia_get_params(lasco_map)
    extent_global = [
        -p_lasco['cx'], p_lasco['nx'] - p_lasco['cx'],
        -p_lasco['cy'], p_lasco['ny'] - p_lasco['cy']
    ]

    lasco_diff = lasco_map.data - lasco_map_base.data
    ...
    theta = _np.linspace(0, 2 * _np.pi, 400)
    lasco_range = dict(inner=2.0, outer=6.0)
    for radius in range(int(lasco_range['inner']), int(lasco_range['outer']) + 1):
        ax.plot(
            scale * radius * _np.cos(theta),
            scale * radius * _np.sin(theta),
            ':',
            color='black',
            linewidth=0.8
        )
    ...
    try:
        delta_minutes = (target_time_obj - base_time_obj).to_value(u.min)
    except Exception:
        delta_minutes = None

    diff_label = f"{delta_minutes:.1f} min" if delta_minutes is not None else f"Base {base_time_obj.iso}"
    title_lines = [
        f"LASCO-C2 Difference ({diff_label})",
        f"Target: {lasco_map.date.strftime('%Y-%m-%d %H:%M:%S')} | Base: {lasco_map_base.date.strftime('%Y-%m-%d %H:%M:%S')}"
    ]
    ax.set_title("\n".join(title_lines))
...
if not hasattr(integrated_analysis, "_original_create_single_diff_from_time_image"):
    integrated_analysis._original_create_single_diff_from_time_image = integrated_analysis.create_single_diff_from_time_image

integrated_analysis.create_single_diff_from_time_image = _create_lasco_only_diff_image
...
if __name__ == "__main__":
    ...
    """
    h_apex=4.6; kappa=0.05; alpha_deg=30; tilt_deg=70; lon_deg=-44; lat_deg=19


    ts = "2022-06-13T03:01:00"
