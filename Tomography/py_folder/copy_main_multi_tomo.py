...
def scale_observation_weights(obs: Observation, scale: float) -> Observation:
    """
    Return a copy of an observation with only the least-squares weights scaled.

    This does not change the pB data and is therefore not a photometric
    calibration.  It only changes how strongly the observation contributes to
    the weighted inverse problem.
    """
    scale = float(scale)
    if not np.isfinite(scale) or scale <= 0:
        raise ValueError(f"Observation weight scale must be positive and finite, got {scale}")
...
def balance_observation_weights_by_group(
    ...
    """
    Balance weighted least-squares contributions between observation groups.

    The inversion objective uses W^2 (A x - y)^2.  When COR1A and Earth-view
    pB are combined, one group can dominate simply because it has more valid
    pixels or systematically different ybk-derived weights.  This routine
    rescales only the observation weights so that each group has approximately
    the same total sum(W^2) as the reference group.

    This is not a pB calibration and does not modify obs.pb.  It is a numerical
    balancing option for mixed-instrument inversions.
    """
    if not observations:
        return observations
    ...
    for i, path in enumerate(pb_paths):
        grouped.setdefault(tomography_observation_group_key(path), []).append(i)

    if len(grouped) <= 1:
        print("[INFO] Group weight balancing skipped: only one observation group is present.")
        return observations

    group_sum_w2: dict[str, float] = {}
    group_npix: dict[str, int] = {}
    for key, indices in grouped.items():
        ww = np.concatenate([np.asarray(observations[i].w, dtype=np.float64) for i in indices])
        ww = ww[np.isfinite(ww) & (ww > 0)]
        group_npix[key] = int(ww.size)
        group_sum_w2[key] = float(np.sum(ww * ww)) if ww.size else np.nan

    reference_group = str(reference_group or "").strip()
    if reference_group in group_sum_w2 and np.isfinite(group_sum_w2[reference_group]) and group_sum_w2[reference_group] > 0:
        target_sum_w2 = float(group_sum_w2[reference_group])
        target_label = reference_group
    else:
        usable = [v for v in group_sum_w2.values() if np.isfinite(v) and v > 0]
        if not usable:
            print("[WARN] Group weight balancing skipped: no usable group weight sums.")
            return observations
        target_sum_w2 = float(np.nanmedian(usable))
        target_label = "median usable group"

    min_factor = float(min_factor)
    max_factor = float(max_factor)
    if not np.isfinite(min_factor) or min_factor <= 0:
        min_factor = 0.25
    if not np.isfinite(max_factor) or max_factor < min_factor:
        max_factor = max(min_factor, 4.0)

    out = list(observations)
    print(
        f"[INFO] Group weight balancing enabled: target={target_label}, "
        f"target_sum_w2={target_sum_w2:.3e}, clamp=[{min_factor:.3g}, {max_factor:.3g}]"
    )

    for key, indices in grouped.items():
        sum_w2 = group_sum_w2.get(key, np.nan)
        if not np.isfinite(sum_w2) or sum_w2 <= 0:
            print(f"[WARN] Group weight balancing skipped for group '{key}': unusable sum_w2={sum_w2}.")
            continue

        factor_raw = np.sqrt(target_sum_w2 / sum_w2)
        factor = float(np.clip(factor_raw, min_factor, max_factor))
        for i in indices:
            out[i] = scale_observation_weights(out[i], factor)

        new_sum_w2 = sum_w2 * factor * factor
        print(
            f"[INFO] Group weight scale for '{key}': "
            f"n_pix={group_npix.get(key, 0)}, sum_w2={sum_w2:.3e}, "
            f"factor_raw={factor_raw:.6g}, factor_used={factor:.6g}, "
            f"new_sum_w2={new_sum_w2:.3e}"
        )

    return out

def _extract_surface_compat(dataset):
    """
    Extract a PyVista surface while keeping behavior stable across PyVista versions.

    PyVista currently warns that the default extract_surface algorithm will change.
    Passing algorithm="dataset_surface" preserves the current behavior and silences
    the warning on versions that support the keyword.  Older versions fall back to
    the previous call signature.
    """
    try:
    ...
    try:
        ...
        for rid in selected_ids:
            ...
            if piece is not None and piece.n_points > 0:
                pieces.append(_extract_surface_compat(piece).triangulate())
        ...
        return _extract_surface_compat(merged).triangulate()
...
if __name__ == "__main__":
    ...
    LAM = 20 #Tomography反転の正則化強度 λ。小さいほど、観測pBにより忠実になりますが、ノイズやartifactも拾いやすくなります
    Q_LOW = 0.0 # 追加の低周波・大規模背景成分除去を行うかどうか
    WIDTH_PIX = 8.0 #pBのpolar sampling時に、半径方向カットの横幅を何ピクセル平均するか
    ...
    BALANCE_GROUP_WEIGHTS = True
    BALANCE_GROUP_REFERENCE = "earth_merged"
    BALANCE_GROUP_MIN_FACTOR = 0.25
    BALANCE_GROUP_MAX_FACTOR = 4.0
    ...
    SAVE_NE_NPZ = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/Rawdata/ne_npz/ne3d_{TARGET_TAG}_{WINDOW_TAG}_{FREQ_TAG}MHz_h{int(HARMONIC)}_Earth_cor1-copy_main_multi_tomo.npz"



    SAVE_PNG_PATH = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/output/tomo_{TARGET_TAG}_{WINDOW_TAG}_{FREQ_TAG}MHz_h{int(HARMONIC)}_Earth_cor1-copy_main_multi_tomo.png"
