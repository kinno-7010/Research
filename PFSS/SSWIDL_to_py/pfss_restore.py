"""
pfss_restore.py - Routine to restore input pfss 'save' file and update pfss common block
...
def pfss_restore(restfile, refresh=False, loud=False, bfield_struct=None):
    """
    Restore input pfss 'save' file and update pfss common block.
...
def pfss_restore_legacy(restfile, refresh=False, loud=False):
    """
    Legacy version of pfss_restore for compatibility.
    
    This maintains the original interface while providing the same functionality.
    """
    return pfss_restore(restfile, refresh=refresh, loud=loud)


# For compatibility with IDL calling convention
def pfss_restore_idl(restfile, refresh=False, loud=False):
    """
    IDL-compatible wrapper for pfss_restore.
    
    This function updates the global data block and doesn't return anything.
    """
    pfss_restore(restfile, refresh=refresh, loud=loud)
