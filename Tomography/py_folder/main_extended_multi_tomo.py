...
def apply_defaults(args: SimpleNamespace) -> SimpleNamespace:
    ...
    defaults = dict(
        ...
        cross_calibration_solver_maxiter=3000,
...
def prepare_tomography_problem(args: SimpleNamespace) -> PreparedProblem:
    ...
    if bool(getattr(args, "thomson_normalize_msb", True)):
        print(
            ...
            f"factor=1/(1-u/3)={base.thomson_msb_normalization_factor(float(args.limb_u)):.6g}"
        )
    else:
        print("[INFO] Thomson kernel MSB normalization disabled; verify the pB unit convention.")
    ...
    density_basis = base.density_basis_from_grid(
        grid, model=str(args.density_prior_model), scale=float(args.density_prior_scale)
    )
...
def save_final_npz(
    ...
    np.savez_compressed(
        ...
        ds=float(args.ds), limb_u=float(args.limb_u), filt=bool(args.filt),
...
def scenario_configuration_hash(args: SimpleNamespace) -> str:
    ...
    fields = [
        ...
        "include_kcor_lasco", "include_cor1a", "include_lasco_only",
...
if __name__ == "__main__":
    ...
    R_MIN, R_MAX = 1.5, 4.0
    NR, NTH, NPH = 64, 64, 128 # NR, NTH, NPH はボクセル分割数（半径方向、緯度方向、経度方向）
    # NR, NTH, NPH = 60,60,120
    ...
    LAMBDA_SCAN_VALUES = [1.0, 5.0, 8.0, 10.0, 15.0]
    LAMBDA_SELECT_MODE = "fixed"
    ...
    FIXED_GROUP_FORWARD_GAINS = {}
    CROSS_CALIBRATION_INITIAL_GAIN_BY_GROUP = {"cor1a": 0.81} # COR1A gainの初期値。設定した値から反復を開始。
    ...
    CROSS_CALIBRATION_SOLVER_TOL = 1e-3 # gain更新用の緩和した収束閾値
    CROSS_CALIBRATION_DAMPING = 0.9 # 新しく推定したgainをどの程度反映するか。設定値：新しい推定値の割合、1-設定値：以前の値の割合 を対数空間で混合
