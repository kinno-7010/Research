#!/usr/bin/env python3
"""
1min_diff統計データに中央値を追加するツール

このスクリプトは、cme_observations_*_1min_diff.csvからheight_rsunデータを読み込み、
時刻ごとの中央値を計算して、対応するcme_statistics_*_1min_diff.csvに追記します。

使用例:
python add_median_to_1min_diff_statistics.py --start_time "2022-06-13T03:00:00" --end_time "2022-06-13T04:00:00"
"""
...
class MedianStatisticsAdder1minDiff:
    """
    1min_diff統計データに中央値を追加するクラス
    """
    ...
    def find_observations_files(self, start_time=None, end_time=None):
        """
        指定時間範囲の1min_diff observations CSVファイルを検索
        """
        pattern = str(self.obs_folder / "cme_observations_*_1min_diff.csv")
        ...
        for file_path in obs_files:
            try:
                # ファイル名から時刻を抽出 (例: cme_observations_220613-032055_1min_diff.csv)
    ...
    def find_statistics_file(self, obs_file_path):
        """
        observations CSVファイルに対応する1min_diff statistics CSVファイルを検索
        """
        ...
        stats_pattern = str(self.csv_folder_path / f"cme_statistics_{time_part}_1min_diff.csv")
        stats_files = glob.glob(stats_pattern)
        
        # sta/ フォルダ内も検索
        sta_pattern = str(self.sta_folder / f"cme_statistics_{time_part}_1min_diff.csv")
    ...
    def process_time_range(self, start_time=None, end_time=None, backup=True):
        """
        指定時間範囲の1min_diffファイルを処理
        """
        print(f"Processing 1min_diff time range: {start_time} - {end_time}")
        
        # observations ファイルを検索
        obs_files = self.find_observations_files(start_time, end_time)
        print(f"Found {len(obs_files)} observations files (1min_diff)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='1min_diff統計データに中央値を追加',
...
使用例:
  python add_median_to_1min_diff_statistics.py --start_time "2022-06-13T03:00:00" --end_time "2022-06-13T04:00:00"
    ...
    try:
        # プロセッサーの初期化
        processor = MedianStatisticsAdder1minDiff(args.csv_folder)
