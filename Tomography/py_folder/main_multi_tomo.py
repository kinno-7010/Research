...
def find_pb_fits_in_time_window(
    ...
    """
    Find pB FITS files within target_time +/- window_days.

    The returned list is sorted by observation time and then filename. Each file is used as an
    independent pB constraint in the regularized tomography. This is appropriate for a
    rotational-tomography-style reconstruction of the quasi-stationary background corona,
    not for temporal despiking across the full two-week interval.
    """
    ...
    if include_kcor_lasco:
        candidates.extend(data_dir.glob("pB_Kcor_LASCO_axi_????????_????.fits"))
    if include_cor1a:
        candidates.extend(data_dir.glob("COR1A_pb_pre_????????_??????.fits"))
...
def build_observation(
    ...
    if pb_override is not None:
        ...
        pb = rebin_idl_linear(pb0, out_n) if orig_n != out_n else pb0.copy()
    pb = np.asarray(pb, dtype=np.float64)
...
def main(args):
    ...
    if args.filt and len(pb_paths) >= 2:
        ...
        for p in pb_paths:
            ...
            pb1 = rebin_idl_linear(pb0, args.out_n) if pb0.shape[0] != args.out_n else pb0
    ...
    if np.count_nonzero(m) > 100:
        ...
        suggested_scale = num / den if den > 0 else 1.0
        if not np.isfinite(suggested_scale) or suggested_scale <= 0:
            suggested_scale = 1.0
    else:
        suggested_scale = 1.0
    ...
    if bool(args.apply_brightness_scale):
        print(f"[INFO] Applied brightness-scale correction: scale={scale:.6g}")
    else:
...
if __name__ == "__main__":
    ...
    # Target-centered rotational-tomography input search.
    # The code searches DATA_DIR for pB files within TARGET_TIME +/- SEARCH_WINDOW_DAYS.
    PB_FITS = []
    ...
    TOL = 1e-4
    APPLY_BRIGHTNESS_SCALE = False
    # For +/- 7 day rotational tomography, do not run a single temporal despike over all files.
    ...
    SAVE_NE_NPZ = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/Rawdata/ne3d_solution_{TARGET_TAG}_{WINDOW_TAG}_{FREQ_TAG}MHz.npz"



    SAVE_PNG_PATH = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/output/tomo_{TARGET_TAG}_{WINDOW_TAG}_{FREQ_TAG}MHz.png"
