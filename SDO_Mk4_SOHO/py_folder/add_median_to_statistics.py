#!/usr/bin/env python3
"""
CME統計データに中央値を追加するツール

このスクリプトは、cme_observations_*_min_diff_with_line.csvからheight_rsunデータを読み込み、
時刻ごとの中央値を計算して、対応するcme_statistics_*_min_diff_with_line.csvに追記します。

機能:
- observations CSVからheight_rsunデータ読み込み
- 時刻ごとの中央値計算
- statistics CSVに中央値行追記
- 既存ファイルの自動バックアップ
- エラーハンドリングとログ出力

使用例:
python add_median_to_statistics.py --start_time "2022-06-13T03:00:00" --end_time "2022-06-13T04:00:00"
"""
...
class MedianStatisticsAdder:
    """
    CME統計データに中央値を追加するクラス
    """
    ...
    def find_observations_files(self, start_time=None, end_time=None):
        """
        指定時間範囲のobservations CSVファイルを検索
        ...
        --------
        list
            observations CSVファイルのリスト
        """
        pattern = str(self.obs_folder / "cme_observations_*_min_diff_with_line.csv")
        ...
        for file_path in obs_files:
            try:
                # ファイル名から時刻を抽出 (例: cme_observations_220613-032055_min_diff_with_line.csv)
                filename = os.path.basename(file_path)
                time_part = filename.split('_')[2]  # 220613-032055
    ...
    def find_statistics_file(self, obs_file_path):
        """
        observations CSVファイルに対応するstatistics CSVファイルを検索
        
        Parameters:
        -----------
        obs_file_path : str
            observations CSVファイルのパス
            
        Returns:
        --------
        str or None
            対応するstatistics CSVファイルのパス
        """
        # observations ファイル名から時刻部分を抽出
        obs_filename = os.path.basename(obs_file_path)
        time_part = obs_filename.split('_')[2]  # 220613-032055
        
        # 対応するstatistics ファイルを検索
        stats_pattern = str(self.csv_folder_path / f"cme_statistics_{time_part}_min_diff_with_line.csv")
        stats_files = glob.glob(stats_pattern)
        
        # sta/ フォルダ内も検索
        sta_pattern = str(self.sta_folder / f"cme_statistics_{time_part}_min_diff_with_line.csv")
    ...
    def add_median_to_statistics(self, stats_file_path, median_info, backup=True):
        ...
        try:
            ...
            print(f"  Median std value: {median_info['median_std_height_rsun']:.3f} Rs")  # 中央値ベース標準偏差を表示
    ...
    def process_time_range(self, start_time=None, end_time=None, backup=True):
        ...
        print(f"Found {len(obs_files)} observations files")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='CME統計データに中央値を追加',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  python add_median_to_statistics.py --start_time "2022-06-13T03:00:00" --end_time "2022-06-13T04:00:00"
  python add_median_to_statistics.py --start_time "2022-06-13T03:00:00" --end_time "2022-06-13T04:00:00" --no_backup
        """
    )
    ...
    try:
        # プロセッサーの初期化
        processor = MedianStatisticsAdder(args.csv_folder)
...
if __name__ == "__main__":
    ...
    if len(os.sys.argv) == 1:
        print("CME統計データ中央値追加ツール")
        print("==============================")
        print()
        print("使用例:")
        print("python add_median_to_statistics.py --start_time '2022-06-13T03:00:00' --end_time '2022-06-13T04:00:00'")
        print("python add_median_to_statistics.py --start_time '2022-06-13T03:00:00' --end_time '2022-06-13T04:00:00' --no_backup")
        print()
        print("オプション:")
        print("  --csv_folder    : CSVフォルダのパス")
        print("  --no_backup     : バックアップを作成しない")
        print()
        print("直接実行例を開始します...")
        print()
        ...
        try:
            processor = MedianStatisticsAdder(csv_folder)
            results = processor.process_time_range(
                "2022-06-13T03:00:00",
                "2022-06-13T04:00:00"
            )
