...
# -*- coding: utf-8 -*-
"""
main_overlay_tomography_gcs_from_npz.py

Load an already-computed tomography solution from an NPZ file, then overlay
tomography isodensity (iso-frequency) surfaces with a GCS shell and PFSS
field lines.
...
------------------------------
numpy, pyvista
astropy + sunpy (needed for coordinate transforms, GCS, and PFSS)
...
def add_runinfo_legend(
    ...
    *,
    ...
    base_lines = [
        ...
        f"     apex=({float(gcs_params['lon_deg']):.1f},{float(gcs_params['lat_deg']):.1f}) deg",
    ]
...
def load_tomography_from_npz(Frequency_MHz: List[float]):
    ...
    by the GCS/Spheroid/PFSS overlay.
...
def main(
    ...
    PNG_PATH = Path(
        ...
        f"tomo_gcs_{''.join(str(f) for f in ISO_FREQ_MHZ)}MHz_{time_iso.replace(':', '')}.png"
    )
    # PNG_PATH = Path(f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography\output\overlay_tomo_gcs_{''.join(str(f) for f in ISO_FREQ_MHZ)}MHz_npz_highres.png")
    ...
    if gcs_info and "surface" in gcs_info:
        add_gcs_surface_at_radius(
            ...
            dr=r_scatter_dr,           # ★帯の厚み（まずは 0.03〜0.10 Rs 程度）
...
if __name__ == "__main__":
    ...
    main(Frequency_MHz=Frequency_MHz, time_iso=time_iso, h_apex=h_apex, kappa=kappa, alpha_deg=alpha_deg, tilt_deg=tilt_deg, lon_deg=lon_deg, lat_deg=lat_deg, rss=rss, r_scatter=r_scatter, r_scatter_dr=r_scatter_dr)
