...
def choose_camera_lonlat_near_target(
    ...
    """
    Choose the visualization camera from the observation closest to target_time.
    This affects only the rendered viewpoint, not the reconstructed 3-D density.
    """
    if not obs_list or not pb_paths:
        return None
    ...
    if best_i is not None and obs_list[best_i].lonlat_deg is not None:
        return obs_list[best_i].lonlat_deg

    for obs in obs_list:
        if obs.lonlat_deg is not None:
            return obs.lonlat_deg
    return None
...
def normalize_lam_cv_values(value) -> List[float]:
    """
    Normalize regularization-parameter candidates for Wang+2025-style
    holdout cross validation.

    Accepted forms:
      - list/tuple/ndarray of positive floats
      - comma-separated string, e.g. "3,10,30,100"
    """
    if value is None:
        return []
    if isinstance(value, str):
...
def balance_observation_weights_by_group(
    obs_list: List[Observation],
    pb_paths: List[Path],
    mode: str = "sumw2",
) -> Tuple[List[Observation], dict[str, float]]:
    """
    Balance the total statistical influence of each observation group.

    This is not needed for a single-instrument Wang+2025-style COR1 rotational
    tomography run.  It is useful only when heterogeneous groups such as
    K-Cor/LASCO and COR1A are combined, because otherwise the group with many
    more valid pixels/rays can dominate the least-squares term.

    mode='sumw2' rescales weights so that each group has the same total
    sum(w^2), which is the natural scale of the weighted residual term.
    """
    if not obs_list or not pb_paths:
        return obs_list, {}

    mode_s = str(mode or "none").strip().lower()
    if mode_s in ("", "none", "false", "off"):
        return obs_list, {}
    if mode_s != "sumw2":
        raise ValueError("Only group weight balance mode 'sumw2' is currently supported.")

    grouped: dict[str, List[int]] = {}
    for i, p in enumerate(pb_paths):
        grouped.setdefault(tomography_observation_group_key(p), []).append(i)

    if len(grouped) <= 1:
        return obs_list, {next(iter(grouped.keys())): 1.0} if grouped else {}

    totals: dict[str, float] = {}
    for key, indices in grouped.items():
        total = 0.0
        for i in indices:
            w = np.asarray(obs_list[i].w, dtype=np.float64)
            good = np.isfinite(w) & (w > 0)
            total += float(np.sum(w[good] * w[good]))
        totals[key] = total

    good_totals = np.array([v for v in totals.values() if np.isfinite(v) and v > 0], dtype=np.float64)
    if good_totals.size == 0:
        print("[WARN] Group weight balancing requested, but no usable group weights were found.")
        return obs_list, {}

    target = float(np.nanmedian(good_totals))
    new_obs = list(obs_list)
    applied: dict[str, float] = {}

    for key, indices in grouped.items():
        total = totals.get(key, np.nan)
        if not np.isfinite(total) or total <= 0:
            applied[key] = 1.0
            continue
        # Weighted objective uses W^2, so multiply W by sqrt(target/total).
        fac = float(np.sqrt(target / total))
        applied[key] = fac
        for i in indices:
            obs = new_obs[i]
            new_obs[i] = Observation(
                pb=obs.pb,
                hdr=obs.hdr,
                x=obs.x,
                y=obs.y,
    ...
    print("[INFO] Group weight balancing applied (mode=sumw2):")
    for key in sorted(applied):
        print(
            f"       {key}: weight_factor={applied[key]:.6g}, "
            f"original_sumw2={totals.get(key, np.nan):.3e}, target_sumw2={target:.3e}"
        )
    return new_obs, applied


def select_lam_by_holdout_cv(
    tomo: "RegularizedTomography",
    y_obs: np.ndarray,
    lam_values: List[float],
    validation_fraction: float = 0.15,
    max_validation_rays: int = 20000,
    random_seed: int = 0,
    maxiter: int = 500,
    tol: float = 1e-3,
) -> Tuple[float, List[Tuple[float, float, int]]]:
    """
    Lightweight Wang+2025-style holdout cross validation for choosing lambda.

    Wang et al. (2025) use n-fold CV by withholding 10--20% of rows of A,
    solving with the remaining rows for multiple regularization parameters, and
    choosing the value that minimizes prediction error on the withheld rows.
    This function implements the same idea with one reproducible holdout split.

    Returns
    -------
    best_lam : float
        Candidate with the smallest weighted validation RMS.
    diagnostics : list of (lam, validation_wrms_rel, cg_info)
        CV diagnostics for all candidates.
    """
    lam_values = normalize_lam_cv_values(lam_values)
    if not lam_values:
        raise ValueError("No positive lambda candidates were supplied for CV.")

    y_obs = np.asarray(y_obs, dtype=np.float64)
    original_W = tomo.W.copy()
    original_lam = float(tomo.lam)

    finite = np.isfinite(y_obs) & np.isfinite(original_W) & (original_W > 0)
    valid_pool = np.flatnonzero(finite)
    if valid_pool.size < 1000:
        print("[WARN] Too few valid rays for lambda CV; using current lambda.")
        return original_lam, []

    rng = np.random.default_rng(int(random_seed))
    n_valid = int(round(float(validation_fraction) * valid_pool.size))
    n_valid = max(100, min(n_valid, int(max_validation_rays), valid_pool.size // 2))
    valid_idx = rng.choice(valid_pool, size=n_valid, replace=False)
    valid_mask = np.zeros(y_obs.size, dtype=bool)
    valid_mask[valid_idx] = True
    train_mask = finite & (~valid_mask)

    diagnostics: List[Tuple[float, float, int]] = []
    print(
        f"[INFO] Lambda holdout CV enabled: n_train={np.count_nonzero(train_mask)}, "
        f"n_valid={np.count_nonzero(valid_mask)}, candidates={lam_values}"
    )
    ...
    try:
        for lam in lam_values:
            W_train = original_W.copy()
            W_train[valid_mask] = 0.0
            tomo.W = W_train
            tomo.lam = float(lam)
            sol, info = tomo.solve(y_obs, maxiter=int(maxiter), tol=float(tol), positivity=True)
            pred = tomo.A_times(sol)

            train_scale = weighted_projection_scale(
                y_obs[train_mask], pred[train_mask], original_W[train_mask], min_count=100
            )
            if not np.isfinite(train_scale) or train_scale <= 0:
                train_scale = 1.0

            yy = y_obs[valid_mask]
            pp = train_scale * pred[valid_mask]
            ww = original_W[valid_mask]
            good = np.isfinite(yy) & np.isfinite(pp) & np.isfinite(ww) & (ww > 0)
            if np.count_nonzero(good) < 100:
                score = np.inf
            else:
                resid = yy[good] - pp[good]
                den = float(np.sum((ww[good] * yy[good]) ** 2))
                score = float(np.sqrt(np.sum((ww[good] * resid) ** 2) / den)) if den > 0 else np.inf

            diagnostics.append((float(lam), float(score), int(info)))
            print(
                f"[CV] lambda={float(lam):.6g}, validation_weighted_rms_rel={score:.5g}, "
                f"train_fit_scale={train_scale:.6g}, cg_info={info}"
            )
            if score < best_score:
                best_score = score
                best_lam = float(lam)
        ...
        tomo.W = original_W
        tomo.lam = original_lam

    print(f"[INFO] Lambda selected by holdout CV: {best_lam:.6g}")
    return best_lam, diagnostics
...
def density_basis_from_grid(
    ...
    on q, the dimensionless correction to the chosen density basis.

    Supported models
    ----------------
    none / flat / absolute
        Solve directly for ne.  This is a formal absolute-density solve, but the
        regularization parameter then has units tied to cm^-3 and must be made
        extremely small.  In this Python implementation it is usually numerically
        poorly scaled unless lambda is re-tuned by many orders of magnitude.

    constant / constant_density / scale
        Use a constant numerical density scale: ne = scale * q.  This does not
        impose a radial density profile; it only non-dimensionalizes the unknown
        so that q is order unity.  This is the recommended Wang-style option for
        this code when one wants to avoid a Saito-shaped prior but still keep the
        Tikhonov parameter in a practical numerical range.

    saito / saito_equatorial
        Use scale * Saito-equatorial(r) as a radial basis.  This is useful for
        stabilizing sparse-view inversions, but it is a stronger prior than the
        constant basis.
    """
    ...
    if model_s in ("constant", "constant_density", "scale", "density_scale", "numerical_scale"):
        basis = np.full_like(rr, scale_f, dtype=np.float64)
    elif model_s in ("saito", "saito_equatorial", "saito1970", "saito1970_equatorial"):
        basis = scale_f * saito_equatorial_ne_cm3(rr)
    else:
        raise ValueError(
            "Unknown density_prior_model="
            f"{model!r}. Use 'none', 'constant', or 'saito_equatorial'."
        )

    basis = np.asarray(basis, dtype=np.float64).ravel(order="C")
    good = np.isfinite(basis) & (basis > 0)
    if not np.all(good):
        raise ValueError(f"density basis {model!r} produced non-positive or non-finite values")
    return basis
...
def smooth_log_density_for_streamer_plot(
    ne3: np.ndarray,
    sigma_vox: Tuple[float, float, float] = (1.0, 2.0, 3.0),
    low_percentile: float = 0.1,
    high_percentile: float = 99.5,
) -> np.ndarray:
    """
    Smooth reconstructed density for streamer-scale visualization.

    The smoothing is applied to log10(ne), not directly to ne, because coronal
    density varies multiplicatively with radius.  This reduces isolated voxel-scale
    artifacts while preserving large-scale streamer-like density enhancements.

    Parameters
    ----------
    ne3 : ndarray
        3-D electron density array with shape (nr, nth, nph).
    sigma_vox : tuple
        Gaussian sigma in voxel units for (r, theta, phi).
        Larger values give smoother isosurfaces.
    low_percentile : float
        Percentile used as a positive density floor before log10.
    high_percentile : float
        Percentile used to clip extreme high-density spikes for plotting.

    Returns
    -------
    ne_smooth : ndarray
        Smoothed positive density array for visualization.
    """
    arr = np.asarray(ne3, dtype=np.float64)
    ...
    vals = arr[pos]
    floor = float(np.nanpercentile(vals, float(low_percentile)))
    ceil = float(np.nanpercentile(vals, float(high_percentile)))

    if not np.isfinite(floor) or floor <= 0:
        floor = float(np.nanmin(vals[vals > 0]))
    if not np.isfinite(ceil) or ceil <= floor:
        ceil = float(np.nanmax(vals))

    clipped = np.where(pos, np.clip(arr, floor, ceil), floor)
    log_ne = np.log10(clipped)

    log_ne_smooth = gaussian_filter(
        log_ne,
        sigma=tuple(float(s) for s in sigma_vox),
        mode=("nearest", "nearest", "wrap"),
    )

    out = 10.0 ** log_ne_smooth
    out[~np.isfinite(out)] = 0.0
    return out


def keep_largest_surface_components(
    surface: pv.PolyData,
    keep_largest_n: int = 1,
    min_component_points: int = 300,
) -> pv.PolyData:
    """
    Keep only the largest connected components of a contour surface.

    This removes tiny disconnected contour fragments that usually correspond to
    voxel-scale tomography artifacts rather than streamer-scale coronal structure.

    Parameters
    ----------
    surface : pv.PolyData
        Input contour surface.
    keep_largest_n : int
        Number of largest connected components to keep.
    min_component_points : int
        Components with fewer points than this are discarded.

    Returns
    -------
    filtered : pv.PolyData
        Filtered surface.
    """
    ...
    try:
        ...
        if "RegionId" in conn.point_data:
            region_ids = np.asarray(conn.point_data["RegionId"], dtype=np.int64)
        elif "RegionId" in conn.cell_data:
            region_ids = np.asarray(conn.cell_data["RegionId"], dtype=np.int64)
        else:
            return surface
        ...
        selected_ids = []
        for idx in order:
            rid = int(unique_ids[idx])
            count = int(counts[idx])
            if count < int(min_component_points):
                continue
            selected_ids.append(rid)
            if len(selected_ids) >= int(keep_largest_n):
                break
        ...
        for rid in selected_ids:
            piece = conn.threshold(
                value=(float(rid) - 0.5, float(rid) + 0.5),
                scalars="RegionId",
            )
            if piece is not None and piece.n_points > 0:
                pieces.append(piece.extract_surface().triangulate())
        ...
        print(f"[WARN] Connected-component filtering skipped: {exc}")
        return surface
# ----------------------------
# Visualization (GUI)
# ----------------------------
def visualize_isosurface(
    ...
    smooth_sigma_vox: Tuple[float, float, float] = (1.5, 3.0, 4.0),
    keep_largest_n_components: int = 1,
    min_component_points: int = 300,
):
    """
    Render isosurfaces specified by plasma frequency (MHz).

    This version is optimized for streamer-scale visualization:
      - optionally smooths log10(ne) before contouring,
      - optionally removes small disconnected contour fragments,
      - keeps the original reconstructed ne unchanged outside this plotting routine.

    Important:
      The smoothing and connected-component filtering are visualization operations.
      For quantitative density or source-height measurements, compare against the
      unsmoothed reconstruction.
    """
    ...
    ne3_raw = np.asarray(ne, dtype=np.float64).reshape((nr, nth, nph), order="C")

    if smooth_for_plot:
        ne3_plot = smooth_log_density_for_streamer_plot(
            ne3_raw,
            sigma_vox=smooth_sigma_vox,
            low_percentile=0.1,
            high_percentile=99.5,
        )
        print(
            "[INFO] Plot-density smoothing enabled: "
            f"log10(ne), sigma_vox={smooth_sigma_vox}, high_percentile=99.5"
        )
    else:
        ne3_plot = ne3_raw
    ...
    ne2 = np.concatenate([ne3_plot, ne3_plot[:, :, :1]], axis=2)

    xx = rr2 * np.sin(tt2) * np.cos(pp2)
    yy = rr2 * np.sin(tt2) * np.sin(pp2)
    zz = rr2 * np.cos(tt2)

    # Frequency field corresponding to the plotted electron density.
    ...
    # Report both raw and plot-smoothed ranges so that the effect is explicit.
    fr_raw = frequency_range_mhz_from_ne(ne3_raw.ravel(order="C"), harmonic=harmonic)
    fr_plot = frequency_range_mhz_from_ne(ne3_plot.ravel(order="C"), harmonic=harmonic)

    if fr_plot is not None:
        ne_min, ne_max, flo, fhi = fr_plot
        has_ne = True
    else:
        ne_min = ne_max = flo = fhi = np.nan
        has_ne = False

    if fr_raw is not None and smooth_for_plot:
        raw_ne_min, raw_ne_max, raw_flo, raw_fhi = fr_raw
        print(
            f"[INFO] Raw f-range before plot smoothing: "
            f"{raw_flo:.3f} .. {raw_fhi:.3f} MHz"
        )
        print(
            f"[INFO] Plot f-range after smoothing: "
            f"{flo:.3f} .. {fhi:.3f} MHz"
        )
    ...
    if has_ne:
        for f, col in zip(freq_list, colors):
            ...
            print(
                f"[INFO] Requested isosurface: f={f:.3f} MHz, "
                f"harmonic={harmonic}, ne={ne_iso:.3e} cm^-3"
            )

            if (not np.isfinite(f)) or (f < flo) or (f > fhi):
                print(
                    f"[WARN] Requested f={f:.3f} MHz is outside plotted reconstruction range "
            ...
            if keep_largest_n_components is not None and int(keep_largest_n_components) > 0:
                before_points = int(contours.n_points)
                contours = keep_largest_surface_components(
                    contours,
                    keep_largest_n=int(keep_largest_n_components),
                    min_component_points=int(min_component_points),
                )
                after_points = int(contours.n_points)
                print(
                    f"[INFO] Connected-component filtering: "
                    f"{before_points} -> {after_points} points "
                    f"(keep_largest_n={keep_largest_n_components}, "
                    f"min_component_points={min_component_points})"
                )

            if contours.n_points == 0:
                print(f"[WARN] Contour removed by connected-component filtering for f={f:.3f} MHz.")
                continue
    ...
    f_label = "Fundamental" if harmonic == 1 else "Second Harmonic"

    if not any_mesh:
        p.add_text(
            "No requested isosurface rendered.\n"
            "Check density scale, smoothing, r_use_min/max, pb_floor, and requested frequency.",
            position="upper_left",
            font_size=12,
            color="black",
        )
    ...
    if has_ne and np.isfinite(flo) and np.isfinite(fhi):
        label_prefix = "Plot-smoothed" if smooth_for_plot else "Reconstructed"
        p.add_text(
            f"{label_prefix} f-range: {flo:.2f} .. {fhi:.2f} MHz ({f_label})",
            position="lower_left",
            font_size=10,
            color="black",
        )
        p.add_text(
            f"{label_prefix} ne range: {ne_min:.3e} .. {ne_max:.3e} cm^-3",
...
def main(args):
    ...
    defaults = dict(
        ...
        balance_group_weights=False,
        group_weight_balance_mode="sumw2",
        use_lam_cv=False,
        lam_cv_values=(3.0, 10.0, 30.0, 100.0),
        lam_cv_fraction=0.15,
        lam_cv_max_validation_rays=20000,
        lam_cv_random_seed=0,
        lam_cv_maxiter=500,
        lam_cv_tol=1e-3,
    ...
    if bool(args.auto_find_pb_fits):
        ...
        found = find_pb_fits_in_time_window(
            ...
            match_cor1a_to_earth=bool(getattr(args, "match_cor1a_to_earth", True)),
            max_cor1a_match_minutes=(
                None if getattr(args, "max_cor1a_match_minutes", None) is None
                else float(getattr(args, "max_cor1a_match_minutes", 90.0))
            ),
        )
    ...
    group_weight_factors = {}
    if bool(getattr(args, "balance_group_weights", False)):
        obs_list, group_weight_factors = balance_observation_weights_by_group(
            obs_list=obs_list,
            pb_paths=pb_paths,
            mode=str(getattr(args, "group_weight_balance_mode", "sumw2")),
        )
    ...
    cv_lam_diagnostics: List[Tuple[float, float, int]] = []
    if bool(getattr(args, "use_lam_cv", False)):
        best_lam, cv_lam_diagnostics = select_lam_by_holdout_cv(
            tomo=tomo,
            y_obs=y_obs,
            lam_values=normalize_lam_cv_values(getattr(args, "lam_cv_values", [])),
            validation_fraction=float(getattr(args, "lam_cv_fraction", 0.15)),
            max_validation_rays=int(getattr(args, "lam_cv_max_validation_rays", 20000)),
            random_seed=int(getattr(args, "lam_cv_random_seed", 0)),
            maxiter=int(getattr(args, "lam_cv_maxiter", 500)),
            tol=float(getattr(args, "lam_cv_tol", 1e-3)),
        )
        tomo.lam = float(best_lam)
        args.lam = float(best_lam)
    ...
    if args.save_ne_npz:
        ...
        np.savez_compressed(
            ...
            group_weight_factor_keys=np.array(list(group_weight_factors.keys()), dtype="U64"),
            group_weight_factor_values=np.array(list(group_weight_factors.values()), dtype=np.float64),
            final_lam=float(args.lam),
            use_lam_cv=bool(getattr(args, "use_lam_cv", False)),
            lam_cv_values=np.array([x[0] for x in cv_lam_diagnostics], dtype=np.float64),
            lam_cv_scores=np.array([x[1] for x in cv_lam_diagnostics], dtype=np.float64),
            lam_cv_cg_info=np.array([x[2] for x in cv_lam_diagnostics], dtype=np.int32),
            pb_files=np.array([str(p) for p in pb_paths], dtype="U512"),
...
if __name__ == "__main__":
    ...
    # Wang+2025-style reference run: single-instrument rotational tomography.
    # Use COR1A only by default.  For a heterogeneous Earth+COR1A test, switch
    # INCLUDE_KCOR_LASCO=True and consider BALANCE_GROUP_WEIGHTS=True.
    INCLUDE_KCOR_LASCO = True
    INCLUDE_COR1A = True
    if INCLUDE_COR1A = True and INCLUDE_KCOR_LASCO = True:
        AUTO_FIND_PB_FITS = True
        PB_FITS = []
    elif INCLUDE_COR1A = True and INCLUDE_KCOR_LASCO = False:
        AUTO_FIND_PB_FITS = False
        PB_FITS = [
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220607_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220607_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220608_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220609_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220610_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220611_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220611_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220612_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220613_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220613_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220614_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220614_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220615_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220616_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220617_025600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220617_210600.fits",
            "/mnt/d/wsl/home/kinno-7010/Research_data/STEREO-A/SECCHI/COR1/pB/Rawdata/COR1A_pb_pre_20220619_210600.fits",
        ]
    elif INCLUDE_COR1A = False and INCLUDE_KCOR_LASCO = True:
        AUTO_FIND_PB_FITS = True
        PB_FITS = []
    else:
        raise ValueError("Invalid combination of INCLUDE_COR1A and INCLUDE_KCOR_LASCO")
    ...
    DEDUPLICATE_PB_FITS = True
    MATCH_COR1A_TO_EARTH = False
    MAX_COR1A_MATCH_MINUTES = None
    ...
    PB_FLOOR = "" #pBの下限値、つまりnoise floorを手動指定するパラメータ。""の場合は自動で決定される

    # Wang+2025 uses low-order PA harmonics for Ibg(r). 1 deg sampling is enough
    # for streamer-scale structure and avoids overweighting fine PA noise.
    DPA_DEG = 1.0 # polar samplingで使うPosition Angle方向の角度刻み
    ...
    R_USE_MAX_BY_GROUP = {}
    PB_SCALE_BY_GROUP = {}
    HM = 3 # Wang+2025: keep PA harmonics k_phi = 0, 1, 2 for Ibg(r)

    WT_NR = 1 # radial regularization weightを使うかどうか
    LAM = 20 # 初期値。最終解析では USE_LAM_CV=True で候補から選ぶことを推奨
    Q_LOW = 0.0 # 追加の低周波・大規模背景成分除去を行うかどうか
    WIDTH_PIX = 10.0 # Wang+2025/Ne3dTomo寄り: Ibg(r)用のradial cutを広めに平均
    ...
    APPLY_BRIGHTNESS_SCALE = False
    # Wang+2025 solves the absolute density with radial weighting, not ne=prior*q.
    # Keep the Saito prior disabled for the Wang-style reconstruction.
    DENSITY_PRIOR_MODEL = "none"
    DENSITY_PRIOR_SCALE = 1.0
    CALIBRATION_REFERENCE_GROUP = "earth_merged"
    ...
    USE_TEMPORAL_DESPIKE = True
    NE3DTOMO_GLOBAL_YBK = True
    # For single-instrument COR1A-only this has no effect.  Enable only for
    # heterogeneous Earth+COR1A tests to prevent one group from dominating by ray count.
    BALANCE_GROUP_WEIGHTS = False
    GROUP_WEIGHT_BALANCE_MODE = "sumw2"
    # Wang+2025 selects the regularization parameter by cross validation.  This
    # implementation is available but disabled by default because it is expensive.
    USE_LAM_CV = False
    LAM_CV_VALUES = [3.0, 10.0, 20.0, 30.0, 100.0]
    LAM_CV_FRACTION = 0.15
    LAM_CV_MAX_VALIDATION_RAYS = 20000
    LAM_CV_RANDOM_SEED = 0
    LAM_CV_MAXITER = 500
    ...
    SAVE_PREPPED_DIR = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/Rawdata/tomo_prepped_{TARGET_TAG}_{WINDOW_TAG}_wang2025_COR1A_only"
    SAVE_NE_NPZ = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/Rawdata/ne_npz/ne3d_{TARGET_TAG}_{WINDOW_TAG}_{FREQ_TAG}MHz_h{int(HARMONIC)}_wang2025_Earth_COR1A.npz"



    SAVE_PNG_PATH = f"/mnt/d/wsl/home/kinno-7010/Research_data/Tomography/output/tomo_{TARGET_TAG}_{WINDOW_TAG}_{FREQ_TAG}MHz_h{int(HARMONIC)}_wang2025_Earth_COR1A.png"
    ...
    args = SimpleNamespace(
        ...
        match_cor1a_to_earth=MATCH_COR1A_TO_EARTH,
        max_cor1a_match_minutes=MAX_COR1A_MATCH_MINUTES,
        ...
        balance_group_weights=BALANCE_GROUP_WEIGHTS,
        group_weight_balance_mode=GROUP_WEIGHT_BALANCE_MODE,
        use_lam_cv=USE_LAM_CV,
        lam_cv_values=LAM_CV_VALUES,
        lam_cv_fraction=LAM_CV_FRACTION,
        lam_cv_max_validation_rays=LAM_CV_MAX_VALIDATION_RAYS,
        lam_cv_random_seed=LAM_CV_RANDOM_SEED,
        lam_cv_maxiter=LAM_CV_MAXITER,
        lam_cv_tol=LAM_CV_TOL,
